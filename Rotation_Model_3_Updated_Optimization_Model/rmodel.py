...
def getData(dict,c,con):    
    ...
    residentsE = {}
    residentsE[0] = {"Sarah", "Kennedy", "Jackson"} #PGY1
    residentsE[1] = {"Jeff", "Anna", "Fran"}
    residentsE[2] = {"Brianna", "Aditi", "Clara"}
    residentsE[3] = {"Prav", "Shayne", "Sam", "Safia"}
    residentsE[4] = {"Sydney", "Tim", "Amanda"} #PGY5
#     for e in classes:
...
def model(m, dict,p_min, p_max):
    ...
    m.setObjective(
    	...
    	# quicksum(v[r,d,w] for e in range(len(dict["classes"])) for r in dict["residentsE"][e] for d in dict["departments"] if d not in dict["departmentsImp"][e] for w in dict["weeksVac"][r]),GRB.MAXIMIZE)
    )
        # print("e: {} in range (len(dict[classes])): {}", e, len(dict[classes]))
